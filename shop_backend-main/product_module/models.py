...
class ProductCategory(models.Model):
    ...
    link = models.CharField(
        verbose_name="لینک مقصد", null=True, blank=True, max_length=100
    )
...
class CategoryBannerGallery(models.Model):
    text = models.CharField(max_length=255, blank=True, default='', )
    category = models.ForeignKey(
        ProductCategory, on_delete=models.CASCADE, related_name="images"
    )
    image = models.ImageField(upload_to="images/category-gallery", blank=True)


class Brand(models.Model):
    title = models.CharField(
        verbose_name="برند محصول", max_length=100, null=True, blank=True
    )
...
class Product(models.Model):
    ...
    image = models.ImageField(
        verbose_name="عکس محصول", upload_to="images/products", null=True, blank=True
    )
    slug = models.SlugField(default="", null=True, blank=True)
    short_description = models.CharField(
        verbose_name="توضیحات کوتاه", null=True, blank=True, max_length=100
    )
    ...
    discounted_price = models.PositiveBigIntegerField(
        verbose_name="قیمت تخفیف خورده", null=True, blank=True
    )
    ...
    created_date = models.DateTimeField(
        verbose_name="تاریخ ثبت محصول", editable=False, auto_now_add=True
    )
...
class ProductProperty(models.Model):
    product = models.ForeignKey(
        "Product", related_name="properties", on_delete=models.CASCADE, default=1
    )
...
class Payment(models.Model):
    ...
    payment_method = models.CharField(
        choices=PaymentMethod.choices, max_length=20, default=PaymentMethod.Wallet
    )
...
class WalletTransaction(models.Model):
    TYPE_CHOICES = [("credit", "واریز"), ("debit", "برداشت"), ("order", "سفارش")]
    METHOD_CHOICES = [
        ("card", "کارت"),
        ("iban", "شبا"),
    ]
