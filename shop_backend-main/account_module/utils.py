...
def is_strong_password(password: str) -> (bool, str):
    ...
    """
    if len(password) < 8:
        return False, "رمز عبور باید حداقل شامل 8 حرف باشد"
    # [A-Z] looks for uppercase
    if not re.search(r'[A-Z]', password):
        return False, "رمز عبور باید شامل حداقل یک حرف بزرگ باشد"
    # [a-z] looks for lowercase
    if not re.search(r'[a-z]', password):
        return False, "رمز عبور باید شامل حداقل یک حرف کوچک باشد"
    ...
    if not re.search(r'[^\w\s]', password):
        return False, "رمز عبور باید حداقل شامل یک نماد مخصوص باشد"
